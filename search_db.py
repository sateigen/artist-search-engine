...
def ask_user():
    ...
    print("4. Exit the Artist Database")
    response = input("What would you like to do? Type 1, 2, 3, or 4 ")
...
def view_database(cur):
    ...
    for row in cur:
        print("\nArtist: {}, Title of Piece: {}, Year: {}, Medium: {}, Subject: {}, Collection: {}".format(row[1], row[2], row[3], row[4], row[5], row[6]))
...
def search_database(cur):
    ...
    if search_criteria in response_dict:
        ...
        cur.execute("SELECT * FROM artists WHERE " + response_dict[search_criteria] + " = %s", (search_item,))
        for row in cur:
